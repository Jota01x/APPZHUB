...
const Dashboard: React.FC<DashboardProps> = ({ setView }) => {
  const [activeRange, setActiveRange] = useState<'hoje' | '7dias' | '30dias' | 'sempre'>('hoje');
  ...
  const getChartData = () => {
    switch (activeRange) {
      ...
      case '30dias':
        return [
          { time: 'SEMANA 01', value: 0 },
          { time: 'SEMANA 02', value: 0 },
          { time: 'SEMANA 03', value: 0 },
          { time: 'SEMANA 04', value: 0 },
        ];
      case 'sempre':
        return [
          { time: 'JAN', value: 0 },
    ...
    <div className="flex flex-col space-y-6 w-full max-w-[1200px] mx-auto py-4">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 px-2">
        <div>
          ...
          <p className="text-zinc-400 font-black text-[10px] uppercase tracking-widest">Métricas de performance em tempo real.</p>
        </div>
      ...
      >
        ...
        <div className="grid grid-cols-2 border-t border-zinc-900 bg-zinc-950/30 mt-10">
      ...
      <div className="flex justify-center pb-12">
        <button 
          ...
          className="px-12 py-5 bg-indigo-600 border border-indigo-500 hover:bg-indigo-700 text-white rounded-full transition-all group flex items-center gap-5 shadow-[0_20px_40px_-10px_rgba(59,130,246,0.3)] active:scale-95"
        >
          <span className="text-[12px] font-black uppercase tracking-[0.4em]">Criar Nova Estrutura</span>
          <div className="p-1 bg-white/10 rounded-lg group-hover:bg-white group-hover:text-indigo-600 transition-all">
            <TrendingUp size={18} />
          </div>
        </button>
      </div>
